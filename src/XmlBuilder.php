...
class XmlBuilder extends RequestBase
{
    ...
    protected function setPayment()
    {
        ...
        if (in_array($type, ['auth', 'sale', 'recurringPayment'])) {
            if ($this->numberOfInstallments > 1) {
                ...
                if ((strlen($this->chargeInterest) > 0) && (!in_array(strtoupper($this->chargeInterest), ['Y', 'N']))) {
    ...
    protected function setRapiRequest()
    {
        ...
        } elseif (in_array(strtolower($this->period), ['today', 'yesterday', 'lastmonth', 'thismonth'])) {
            $this->xml->request->filterOptions->addChild('period', $this->period);
