...
class Request extends XmlBuilder {
    ...
    protected function sendXml() {
        ...
        if (is_object(RequestBase::$logger)) { self::$logger->logInfo('Sending XML to '.$this->endpoint); }
        $curlInfo = curl_getinfo($curl);
        curl_close($curl);
        if (RequestBase::$debug == true) { $this->printDebug($curlInfo); }
    ...
    private function parseXml($array = array(),$c = 0) {
        ...
        if (is_object(RequestBase::$logger)) { self::$logger->logNotice('Parsed parameters received', $array); }
        return $array;
    }
