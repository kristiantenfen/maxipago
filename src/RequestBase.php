...
class RequestBase {
    ...
    public static function clearForLog($text) {
        ...
        elseif (is_array($text)) {
            ...
            if (ServiceBase::checkCreditCard($text["number"])) { $text["number"] = str_ireplace($text["number"], substr_replace($text["number"], str_repeat('*',strlen($text["number"])-4),'4'), $text["number"]); }
            return $text;
        }
